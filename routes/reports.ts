...
router.post('/', async (req, res, next) => {
    ...
    console.log(reports)
    for (const report of reports) {
        await DBRequest(`INSERT INTO reports (reportid, userid, body) VALUES (${report.rrd_id}, ${users[0].userid}, '${JSON.stringify(JSON.parse(JSON.stringify(report)))}')`)
            .catch(async (error) => {
                ...
                if (JSON.stringify(currentReport[0].body) !== JSON.stringify(report)) {
                    await DBRequest(`UPDATE reports SET body = '${JSON.stringify(report)}' WHERE reportid = ${report.rrd_id}`)
                        .catch((error) => {
                            console.log(error)
                        })
                }
