...
router.post('/', async (req, res, next) => {
    ...
    const users = await DBRequest(`SELECT * FROM \`users\` WHERE \`users\`.\`api_key\` = ${parseInt(req.query.api_key as string)}`) as any
    if (users.length === 0) {
        SendError(res, "Пользователя с таким api_key не существует")
        return;
    }

    await DBRequest(`INSERT INTO \`reports\` (\`body\`, \`userid\`) VALUES ('${response}', ${users.userid}`).then(() => {
        res.send({
